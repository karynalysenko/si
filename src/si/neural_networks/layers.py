...
class Dropout(Layer):
    ...
    def backward_propagation(self, output_error: np.ndarray) -> np.ndarray:
        """
        Perform backward propagation on the given output error.
        Multiply the received error by the mask.
...
if __name__ == '__main__':
    dropoutLayer = Dropout(probability=0.5)

    input = np.random.rand(10, 5) 

    output_data = dropoutLayer.forward_propagation(input, training=True)
    print("Forward Propagation:\n", output_data)

    output_error = np.random.rand(*output_data.shape)

    backward_output_error = dropoutLayer.backward_propagation(output_error)
